...
如果镜像内声明了持久化目录，创建容器时 Docker 会自动新建匿名存储卷来挂载目录。
大量匿名存储卷不仅影响观感，容器销毁时也难以判断存储卷的归属。

如未手动指定挂载目录，DPanel 会根据【站点标识+目录名】命名存储卷，避免 Docker 生成匿名存储卷。

如需挂载这些目录，可配置【存储卷】或【宿主机文件/目录】。
区别在于：存储卷会在创建容器时将容器内的数据同步至存储卷，挂载目录则不会。
...
将宿主机 IP 及宿主机所在网络中的主机 IP 注入到容器的 `/etc/hosts` 配置中。
注入宿主机 IP 时应使用 `host-gateway`。
