...
对于非面板创建的 compose 项目，通过 DPanel 面板进行管理时，需确保面板可以访问到相关的 compose.yaml 文件。

可通过挂载单个 yaml 文件或将整个 compose 目录挂载到面板容器中。

:::tip
将整个目录挂载到面板容器中需符合面板的目录规范，具体查看[通过挂载存储路径的方式创建](/manual/compose-create#mount)。
:::
...
在面板的 `/dpanel/compose` 目录中新建外部任务目录及 yaml 文件。新建目录时需注意保持与外部任务名称统一。

```
/dpanel
├─ /compose
│  ├─ /my-compose
│  │  └─ compose.yaml  # yaml 文件名只支持 compose.yaml docker-compose.yaml
│  └─ ...
└─ ....
```
